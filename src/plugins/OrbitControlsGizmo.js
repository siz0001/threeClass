...
class OrbitControlsGizmo {
	constructor(orbitControls, options) {
		...
		options = Object.assign({
			size: 200,
			padding: 0,
			bubbleSizePrimary: 11,
			bubbleSizeSecondary: 11,
			lineWidth: 2,
			fontSize: "12px",
			fontFamily: "arial",
			fontWeight: "bold",
    ...
    function createAxes () {
      ...
      return [
        { axis: "x", direction: new Vector3(1, 0, 0), size: size.primary, color: colors.x[0], line, label: "동", position: new Vector3(0, 0, 0)  },
        { axis: "y", direction: new Vector3(0, 1, 0), size: size.primary, color: colors.y[0], line, label: "상", position: new Vector3(0, 0, 0)  },
        { axis: "z", direction: new Vector3(0, 0, 1), size: size.primary, color: colors.z[0], line, label: "남", position: new Vector3(0, 0, 0)  },
        { axis: "-x", direction: new Vector3(-1, 0, 0), size: size.secondary, color: colors.x[1],  line, label: "서", position: new Vector3(0, 0, 0) },
        { axis: "-y", direction: new Vector3(0, -1, 0), size: size.secondary, color: colors.y[1], line, label: "하", position: new Vector3(0, 0, 0) },
        { axis: "-z", direction: new Vector3(0, 0, -1), size: size.secondary, color: colors.z[1], line, label: "북", position: new Vector3(0, 0, 0) },
      ];
    }
    ...
    function drawLayers ( clear ) {
      ...
      for(let i = 0, length = axes.length; i < length; i ++) {
        ...
        const color = (axis.position.z >= -0.01)
          ? axis.color[0]
          : axis.color[1];
          움직임에 따라 반응형 : drawlin이랑 drawcircle의 axis.color를 color로 바꾸고, 위에서 create의 color의 배열을 0,1로 선택하게 한걸 빼주면 됨
   */
  
        // Draw the line that connects it to the center if enabled
        if (axis.line)
        drawLine(center, axis.position, axis.line, axis.color);
  
        // Draw the circle for the axis
        drawCircle(axis.position, axis.size, highlight ? "#000000" : axis.color);
